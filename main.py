...
WelcomeOpt.append(slash.Option(description='ID of your guild',type=3,required=True))
...
GoodbyeOpt.append(slash.Option(description='ID of your guild',type=3,required=True))
...
@bot.slash_cmd()
async def welcomeset(ctx: slash.Context, guild: WelcomeOpt[0], channel: WelcomeOpt[1], message: WelcomeOpt[2]):
    """Set a welcoming message for new members"""
    cur.execute("INSERT OR REPLACE INTO Welcome VALUES (?,?,?);",(guild, channel.id, message))
    await ctx.respond("Welcome message added!")

@bot.slash_cmd()
async def goodbyeset(ctx: slash.Context, guild: GoodbyeOpt[0], channel: GoodbyeOpt[1], message: GoodbyeOpt[2]):
    """Set a goodbye  message for leaving members"""
    cur.execute("INSERT OR REPLACE INTO Goodbye VALUES (?,?,?);",(guild, channel.id, message))
